...
bool PKG::IsPatchPkg() const {
    const auto has_flag = [this](PKGContentFlag flag) {
        return isFlagSet(pkgheader.pkg_content_flags, flag);
    };

    return has_flag(PKGContentFlag::FIRST_PATCH) || has_flag(PKGContentFlag::PATCHGO) ||
           has_flag(PKGContentFlag::SUBSEQUENT_PATCH) || has_flag(PKGContentFlag::DELTA_PATCH) ||
           has_flag(PKGContentFlag::CUMULATIVE_PATCH);
}
        ...
        if (uroot_reached) {
            for (int i = 0; i < 0x10000; i += ent_size) {
                ...
                if (dirent.ino != 0) {
                    ...
                    const std::string extract_dir_name = extract_path.filename().string();
                    const bool extract_dir_has_patch_suffix =
                        extract_dir_name.size() >= 6 &&
                        extract_dir_name.compare(extract_dir_name.size() - 6, 6, "-patch") == 0;

                    if (parent_path.filename() != title_id && !extract_dir_has_patch_suffix) {
